Building DRiyal Core with Visual Studio
========================================

Introduction
---------------------
Solution and project files to build DRiyal Core with `msbuild` or Visual Studio can be found in the `build_msvc` directory. The build has been tested with Visual Studio 2019 (building with earlier versions of Visual Studio should not be expected to work).

To build DRiyal Core from the command-line, it is sufficient to only install the Visual Studio Build Tools component.
...
Qt
---------------------
To build DRiyal Core with the GUI, a static build of Qt is required.
...
To build DRiyal Core without Qt, unload or disable the `driyal-qt`, `libdriyal_qt` and `test_driyal-qt` projects.
...
```cmd
msbuild -property:Configuration=Release -maxCpuCount -verbosity:minimal driyal.sln
```

Alternatively, open the `build_msvc/driyal.sln` file in Visual Studio 2019.

Security
---------------------
[Base address randomization](https://docs.microsoft.com/en-us/cpp/build/reference/dynamicbase-use-address-space-layout-randomization?view=msvc-160) is used to make DRiyal Core more secure. When building DRiyal using the `build_msvc` process base address randomization can be disabled by editing `common.init.vcproj` to change `RandomizedBaseAddress` from `true` to `false` and then rebuilding the project.

To check if `driyald` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/driyald.exe
```
