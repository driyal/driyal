...
def parse_config_into_DRI_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"DRiyal Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/driyal_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/driyal_config.h'), "w", encoding="utf8") as DRI_config:
        DRI_config.writelines(template)

def main():
    parser = argparse.ArgumentParser(description='DRiyal-core msbuild configuration initialiser.')
        ...
        with open(vcxproj_filename + '.in', 'r', encoding='utf-8') as vcxproj_in_file:
            with open(vcxproj_filename, 'w', encoding='utf-8') as vcxproj_file:
                vcxproj_file.write(vcxproj_in_file.read().replace(
                    '@SOURCE_FILES@\n', content))
    parse_config_into_DRI_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/driyal_config.h'), os.path.join(SOURCE_DIR, 'config/driyal-config.h'))
