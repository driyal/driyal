...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/driyal.conf are parsed in qt/driyal.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  driyald [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see driyald -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for driyald but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect driyald signal handlers
    noui_connect();
