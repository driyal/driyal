...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<DRiyalAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_driyal-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_driyal-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_driyal-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = DRiyalUnits::formatWithUnit(unit, balance, false, DRiyalUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    DRiyalAmountField* amountInput = receiveCoinsDialog.findChild<DRiyalAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_driyal-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
