...
#include <qt/driyaladdressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DRiyal address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new DRiyalAddressEntryValidator(parent));
    widget->setCheckValidator(new DRiyalAddressCheckValidator(parent));
}
...
bool parseDRiyalURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no driyal: URI
    if(!uri.isValid() || uri.scheme() != QString("driyal"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DRiyalUnits::parse(DRiyalUnits::DRI, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDRiyalURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseDRiyalURI(uriInstance, out);
}

QString formatDRiyalURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("driyal:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DRiyalUnits::format(DRiyalUnits::DRI, info.amount, false, DRiyalUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openDRiyalConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", DRIYAL_CONF_FILENAME));
    ...
    /* Open driyal.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DRiyal.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DRiyal (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("DRiyal (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for DRiyal*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "driyal.desktop";
    return GetAutostartDir() / strprintf("driyal-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a driyal.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=DRiyal\n";
        else
            optionFile << strprintf("Name=DRiyal (%s)\n", chain);
