dnl Copyright (c) 2013-2015 The DRiyal Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

AC_DEFUN([DRIYAL_FIND_BDB48],[
  ...
  if test "$use_bdb" = "no"; then
    ...
    if test "$bdbpath" = "X"; then
      ...
      DRIYAL_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdbpath}],db_cxx)
      ...
      DRIYAL_SUBDIR_TO_INCLUDE(BDB_CPPFLAGS,[${bdb48path}],db_cxx)
      bdbpath="${bdb48path}"
      use_bdb=yes
    fi
  else
