DRiyal Core version 0.11.0 is now available from:

  <https://driyal.org/bin/driyal-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer DRI/kB fee to be
...
For example, add the following to `driyal.conf`:
...
related to the blockchain in the driyal system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows DRiyal Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that DRiyal Core will 
...
For an example script see [driyal-submittx](https://github.com/laanwj/driyal-submittx).
...
- #5334 `cf87536` libdriyalconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new driyal-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only DRiyalAmountField
- #5753 `0f44672` Add driyal logo to about screen
...
- #5809 `46bfbe7` Add driyal-cli man page
...
- #5849 `41259ca` contrib/init/driyald.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename DRiyal-Qt.app to DRiyal-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for driyal-tx)
