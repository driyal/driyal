DRiyal Core version 0.13.1 is now available from:

  <https://driyal.org/bin/driyal-core-0.13.1/>
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming DRiyal wallet
  software more difficult and which seriously complicates the design of smart
  contracts for DRiyal.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each DRiyal block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of driyals they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of driyals being spent is exactly
...
- **Increased security for multisig:** DRiyal addresses (both P2PKH addresses
  ...
  believed to be provided by DRiyal's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  DRiyal paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of DRiyal's current ledger (called the UTXO set),
  ...
  note that DRiyal Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  DRiyal users to individually opt-in to almost any change in the DRiyal
  Script language when those users receive new transactions.  Features
  currently being researched by DRiyal Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other DRiyal community members are studying several other improvements
...
[Segwit FAQ]: https://driyalcore.org/en/2016/01/26/segwit-benefits/
[segwit wallet developers guide]: https://driyalcore.org/en/segwit_wallet_dev/
[BIP141]: https://github.com/driyal/bips/blob/master/bip-0141.mediawiki
[BIP143]: https://github.com/driyal/bips/blob/master/bip-0143.mediawiki
[BIP144]: https://github.com/driyal/bips/blob/master/bip-0144.mediawiki
[BIP145]: https://github.com/driyal/bips/blob/master/bip-0145.mediawiki
[versionbits FAQ]: https://driyalcore.org/en/2016/06/08/version-bits-miners-faq/
...
Since DRiyal Core 0.10.0, nodes have defaulted to only relaying and
...
[BIP147]: https://github.com/driyal/bips/blob/master/bip-0147.mediawiki
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in driyalconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.driyalstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libdriyalconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_driyal by itself (luke-jr)
...
- #8829 `2a8bca4` Add driyal-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` driyal-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_driyalds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name driyal.qrc  (driyalsSG)
...
- #8817 `bcf3806` update driyal-tx to output witness data (jnewbery)
