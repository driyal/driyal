...
DRiyal Core version 0.20.0 is now available from:

  <https://driyalcore.org/bin/driyal-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from DRiyal Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since DRiyal Core version 0.18.0.
...
- Testing or debugging of implementations of the DRiyal P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of DRiyal Core. DRiyal Core logs debug messages
...
- OpenSSL is no longer used by DRiyal Core.  (#17265)

- BIP70 support has been fully removed from DRiyal Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/driyal/driyal/issues/16599),
  [PR #16702](https://github.com/driyal/driyal/pull/16702), and the
  `driyald help` for more information.  This option is experimental and
...
- All custom settings configured when DRiyal Core starts are now
...
- The "Start DRiyal Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://driyalcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple DRiyal Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed DRiyal Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/driyal-core/HWI) for
  signing.  Future versions of DRiyal Core should support a GUI option
...
- DRiyal Core's automatically-generated source code documentation is
  now available at https://doxygen.driyalcore.org.  (#17596)
...
- The `driyal-cli` utility used with the `-getinfo` parameter now
...
- The `driyal-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `driyalstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_driyal-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBDRIYAL_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_driyal` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBDRIYAL_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for driyal-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate driyal schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest driyal.conf (sdaftuar)
...
- #18022 Fix appveyor `test_driyal` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (driyal-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone driyal-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about DRiyal Core files (hebasto)
...
- #17119 Fix broken driyal-cli examples (andrewtoth)
...
- #17503 Remove bitness from driyal-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.driyalcore.org (laanwj)
...
- #17648 Rename wallet-tool references to driyal-wallet (hel-o)
...
- #18341 Replace remaining literal DRI with `CURRENCY_UNIT` (domob1812)
...
- #16115 On driyald startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from driyal-cli -getinfo (malevolent)
