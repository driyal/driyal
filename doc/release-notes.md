*After branching off for a major version release of DRiyal Core, use this
...
https://github.com/driyal-core/driyal-devwiki/wiki/
...
DRiyal Core version *version* is now available from:

  <https://driyalcore.org/bin/driyal-core-*version*/>
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  DRiyal
...
- A driyald node will no longer rumour addresses to inbound peers by default.
...
- The top-level fee fields `fee`, `modifiedfee`, `ancestorfees` and `descendantfees`
  ...
  fields in the `fees` object are denominated in DRI. (#22689)
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. DRiyal Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from DRiyal Core in 22.0. (#22544)
