...
This guide describes how to build driyald, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build DRiyal Core from source.
...
### 4. Clone DRiyal repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the DRiyal Core repository to a directory.
...
It is not necessary to build wallet functionality to run `driyald` or  `driyal-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/driyal/driyal/blob/master/doc/descriptors.md), `sqlite` is required.
...
DRiyal Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/driyal/driyal/issues/7714).
...
You can deploy a `.dmg` containing the DRiyal Core application using `make deploy`.
...
## Building DRiyal Core

### 1. Configuration

There are many ways to configure DRiyal Core, here are a few common examples:
...
Run the following in your terminal to compile DRiyal Core:
...
DRiyal Core should now be available at `./src/driyald`.
If you compiled support for the GUI, it should be available at `./src/qt/driyal-qt`.

The first time you run `driyald` or `driyal-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/DRiyal/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/DRiyal"

touch "/Users/${USER}/Library/Application Support/DRiyal/driyal.conf"

chmod 600 "/Users/${USER}/Library/Application Support/DRiyal/driyal.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/DRiyal/debug.log
```

## Other commands:

```shell
./src/driyald -daemon      # Starts the driyal daemon.
./src/driyal-cli --help    # Outputs a list of command-line options.
./src/driyal-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/driyal-qt -server # Starts the driyal-qt server mode, allows driyal-cli control
```
