# PSBT Howto for DRiyal Core

Since DRiyal Core 0.17, an RPC interface exists for Partially Signed DRiyal
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/driyal/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for DRiyal transactions that are not fully signed
...
#### Multisig with multiple DRiyal Core instances
...
DRiyal Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `driyal-cli` is used.
...
Later, when *V* DRI has been received on *Amulti*, and Bob and Carol want to
