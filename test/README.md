This directory contains integration tests that test driyald and its
...
driyald and driyal-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (driyal-util, driyal-tx, ...).
...
Before tests can be run locally, DRiyal Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the driyald nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another driyald
process running on the system (perhaps from a previous test which hasn't successfully
killed all its driyald nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other driyald processes are running.

On linux, the test framework will warn if there is another
driyald process running when the tests are started.

If there are zombie driyald processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
driyald processes running on the system, so should not be used if any non-test
driyald processes are being run.**
...
sure driyald processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and driyald `debug.log`s will all be dumped
...
`test_framework.log` and driyald `debug.log`s can be combined into a single
...
call methods that interact with the driyald nodes-under-test.

If further introspection of the driyald instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/driyald.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/driyald <pid>
```
