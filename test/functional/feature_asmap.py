#!/usr/bin/env python3
# Copyright (c) 2020-2021 The DRiyal Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test asmap config argument for ASN-based IP bucketing.

Verify node behaviour and debug log when launching driyald in these cases:

1. `driyald` with no -asmap arg, using /16 prefix for IP bucketing

2. `driyald -asmap=<absolute path>`, using the unit test skeleton asmap

3. `driyald -asmap=<relative path>`, using the unit test skeleton asmap

4. `driyald -asmap/-asmap=` with no file specified, using the default asmap

5. `driyald -asmap` restart with an addrman containing new and tried entries

6. `driyald -asmap` with no file specified and a missing default asmap file

7. `driyald -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(DRiyalTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test driyald with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_asmap_with_absolute_path(self):
        self.log.info('Test driyald -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test driyald -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test driyald {arg} (using default map file)')
            self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test driyald -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
        shutil.copyfile(self.asmap_raw, self.default_asmap)
    ...
    def test_default_asmap_with_missing_file(self):
        self.log.info('Test driyald -asmap with missing default map file')
        self.stop_node(0)
        msg = f"Error: Could not find asmap file \"{self.default_asmap}\""
        self.node.assert_start_raises_init_error(extra_args=['-asmap'], expected_msg=msg)

    def test_empty_asmap(self):
        self.log.info('Test driyald -asmap with empty map file')
        self.stop_node(0)
