...
TMPDIR_PREFIX = "driyal_func_test_"
...
class DRiyalTestMetaClass(type):
    """Metaclass for DRiyalTestFramework.

    Ensures that any attempt to register a subclass of `DRiyalTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'DRiyalTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("DRiyalTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("DRiyalTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class DRiyalTestFramework(metaclass=DRiyalTestMetaClass):
    """Base class for a driyal test script.

    Individual driyal test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave driyalds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop driyalds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use driyal-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_driyald = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "driyald" + config["environment"]["EXEEXT"],
        )
        fname_driyalcli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "driyal-cli" + config["environment"]["EXEEXT"],
        )
        self.options.driyald = os.getenv("DRIYALD", default=fname_driyald)
        self.options.driyalcli = os.getenv("DRIYALCLI", default=fname_driyalcli)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: driyalds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("DRiyalRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'driyald', self.options.driyald) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'driyal-cli', self.options.driyalcli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            if not test_node_i.version_is_at_least(170000):
                ...
                conf_file = test_node_i.driyalconf
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a driyald test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple driyald test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as driyald's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    driyald=self.options.driyald,
                    driyal_cli=self.options.driyalcli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in driyal.conf
    ...
    def skip_if_no_driyald_zmq(self):
        """Skip the running test if driyald has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("driyald has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if driyal-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("driyal-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if driyal-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("driyal-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether driyal-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether driyal-wallet was compiled."""
