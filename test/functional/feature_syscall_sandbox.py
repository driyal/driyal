...
"""Test driyald aborts if a disallowed syscall is used when compiled with the syscall sandbox."""

from test_framework.test_framework import DRiyalTestFramework, SkipTest


class SyscallSandboxTest(DRiyalTestFramework):
    def set_test_params(self):
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        if not self.is_syscall_sandbox_compiled():
            raise SkipTest("driyald has not been built with syscall sandbox enabled.")
        if self.options.nosandbox:
            raise SkipTest("--nosandbox passed to test runner.")

    def run_test(self):
        disallowed_syscall_terminated_driyald = False
        ...
        with self.nodes[0].assert_debug_log([expected_log_entry]):
            ...
            try:
                ...
                disallowed_syscall_terminated_driyald = True
        assert disallowed_syscall_terminated_driyald
        self.nodes = []
