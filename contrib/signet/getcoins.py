...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing driyal-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='driyal-cli', help='driyal-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='DRiyal address to which the faucet should send')
...
parser.add_argument('driyal_cli_args', nargs='*', help='Arguments to pass on to driyal-cli (default: -signet)')

args = parser.parse_args()

if args.driyal_cli_args == []:
    args.driyal_cli_args = ['-signet']


def driyal_cli(rpc_command_and_params):
    argv = [args.cmd] + args.driyal_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = driyal_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = driyal_cli(['getnewaddress', 'faucet', 'bech32'])
