...
Tracing (USDT) support for DRiyal Core on Linux using. For more information on
USDT support in DRiyal Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `driyald` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `driyald` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[DRiyal Core USDT documentation] on how to list available tracepoints in your
`driyald` binary.

[DRiyal Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/driyald
```
...
DRiyal P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/driyald
```
...
In a different terminal, starting DRiyal Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/driyald -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/driyald
```
