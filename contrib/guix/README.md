# Bootstrappable DRiyal Core Builds

This directory contains the files necessary to perform bootstrappable DRiyal
Core builds.
...
To build DRiyal Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      DRiyal Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/driyal-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/driyal-core/guix.sigs).
